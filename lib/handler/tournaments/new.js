...
module.exports.post = function post(req, res) {
  ...
  if(!region) {
    res.status(400).send("Please specify a region");
    return;
  }
  region = region.toLowerCase();

  if(riotRequest.REGIONS.indexOf(region) === -1) {
    res.status(400).send("Please specify a valid region");
    return;
  }
