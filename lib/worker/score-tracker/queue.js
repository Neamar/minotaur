  ...
  var refillQueueWorker = newrelic.createBackgroundTransaction('score-tracker:refillQueue', 'workers', function queueRefillerWorker(job, cb) {
    ...
    async.waterfall([
      ...
      function findParticipants(tournaments, cb) {
        log("Active tournaments: "  + tournaments.length);

        var ids = tournaments.map((t) => t._id);

        mongoose.model("Participant").find({tournament: {$in: ids}}).read('secondaryPreferred').select('_id summonerId region lastKnownGameId');
      },
      function addParticipantsToQueue(participants, cb) {
      ...
      function sendMetrics(cb) {
        ...
        metricTracker("Worker.PushNotifier.participantsCounter", participantsCount);
