...
module.exports = newrelic.createBackgroundTransaction('score-tracker:checkInGame', 'workers', function queueWorker(token, cb) {
  var notified = false;
  ...
  async.waterfall([
    ...
    function sendNotification(res, cb) {
      ...
      // And track on graphite
      metricTracker("Worker.PushNotifier.Notified", 1);
    ...
    var finishTransaction = function(err) {
      ...
      // Note: the error is not forwarded to Redis, otherwise we'd fill Redis very quickly with Timeouts notification
      cb(null, notified);
    };
